...
| CLI schema | `src/cli/schema.rs` | Machine-readable command metadata generated from the live clap tree, including structured command examples |
...
- `volumeleaders-agent schema` emits machine-readable discovery metadata from `Cli::command()` so command paths, help text, structured examples, aliases, auth requirements, and stable argument names cannot drift from clap definitions.
- CLI drift tests assert every visible clap leaf appears in `commands` and `schema`, every leaf has examples, structured schema examples stay valid, aliases retain canonical preferred paths with explicit alias metadata, and global flags appear in schema metadata.
